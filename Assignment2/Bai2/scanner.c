...
void skipComment() {
  // Khi vào hàm này, ta đã đọc được '(*' hoặc '//'
  if (charCodes[currentChar] == CHAR_SLASH) {
    // Comment kiểu C++: // đến cuối dòng
    while (currentChar != '\n' && currentChar != EOF) {
      readChar();
    }
    if (currentChar == '\n') readChar();
  ...
  while (1) {
    ...
    if (charCodes[currentChar] == CHAR_TIMES) { // Gặp dấu *
      readChar();
      if (currentChar == EOF) {
        error(ERR_ENDOFCOMMENT, lineNo, colNo);
        return;
      }
      if (charCodes[currentChar] == CHAR_RPAR) { // Gặp dấu )
        readChar();
...
Token* readConstChar(void) {
  ...
  readChar(); // Đọc tiếp ký tự sau nội dung
  if (currentChar == EOF) {
    token->tokenType = TK_NONE;
    error(ERR_INVALIDCHARCONSTANT, token->lineNo, token->colNo);
    return token;
...
Token* getToken(void) {
  ...
  // Bỏ qua newline (không in ra token)
  if (currentChar == '\n') {
    readChar();
    return getToken();
  }
